...
// EXACT function names from Etherscan Read Contract
const ABI = [
...
const THREADS = 12; // Balanced — faster than 4 but stable on Railway free tier
// ────────────────────────────────────────────────────────
...
async function mineForWallet(wallet, idx) {
  ...
  while (true) {
    try {
      // Fetch challenge and difficulty using EXACT function names
      ...
      console.log(`[W${idx + 1}] Epoch blocks left: ${blocksLeft}`);
      console.log(`[W${idx + 1}] Mining with ${THREADS} threads...`);

      const startTime = Date.now();
      let totalTries = 0;

      // Epoch timer — refresh when epoch rotates (~blocksLeft * 12s)
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        for (let t = 0; t < THREADS; t++) {
          ...
          w.on("error", (e) => console.error(`[W${idx + 1}] Worker err:`, e.message));
        }
      });
...
async function main() {
  ...
  if (!active.length) {
    console.error("❌ No private keys found! Set PK_1 in Railway env variables.");
    process.exit(1);
  }
